    ...
    SmohanMusic.prototype = {
        ...
        createListDom : function(){
            ...
            for(i;i<this.musicLength;i++){
                ul += 
					'<li class="f-toe">'+
						'<strong>'+this.musicList[i]['title']+'</strong> -- '+
						'<small>'+this.musicList[i]['singer']+ '</small>'+
						'<input type="hidden" value="'+i+'">'+
						'<a id="delete"  style="display:none;float:right;color:black;font-size:24px;margin-right:8px;text-decoration: none;">x</a>'+
					'</li>';
            }
            ul += '</ul>';
        ...
        action : function(){
            var  me = this, 
            v = this.musicDom.volume,
            btn = this.musicDom.button;
